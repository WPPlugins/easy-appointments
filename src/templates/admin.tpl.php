...
<script type="text/template" id="ea-tpl-connection-row">
	<td colspan="4" class="table-row-td">
		...
		<p> 
			<strong>
				<%= (row.location == 0) ? '-' : _.findWhere(locations, {id:row.location}).name  %>
			</strong>
		</p>
		<p>
			<strong>
				<%= (row.service == 0) ? '-' : _.findWhere(services, {id:row.service}).name  %>
			</strong>
		</p>
		<p>
			<strong>
				<%= (row.worker == 0) ? '-' : _.findWhere(workers, {id:row.worker}).name  %>
			</strong>
		</p>
	</td>
	<% var weekdays = {
...
<script type="text/template" id="ea-tpl-custumize">
	<div class="wp-filter">
		...
		<table class="form-table form-table-translation">
			<tbody>
				...
				<tr>
					...
					<td>
						<input style="width: 50px; margin-right: 20px;" class="field" data-key="max.appointments" name="max.appointments" type="text" value="<%= _.findWhere(settings, {ea_key:'max.appointments'}).ea_value %>"><br>
					</td>
					<td>
						<span class="description"> <?php _e('Number of appointments that one visitor can make reservation before limit alert is shown. Appointments are count during one day.', 'easy-appointments');?></span>
					</td>
				</tr>
				<tr>
					<th class="row">
				...
				<tr>
					...
					<td>
						<select class="field" name="ea-select-status" data-key="default.status">
							<option value="pending" <% if (_.findWhere(settings, {ea_key:'default.status'}).ea_value == "pending") { %>selected="selected"<% } %>><%= eaData.Status.pending %></option>
							<option value="confirmed" <% if (_.findWhere(settings, {ea_key:'default.status'}).ea_value == "confirmed") { %>selected="selected"<% } %>><%= eaData.Status.confirmed %></option>
						</select>
					</td>
					<td>
		...
		<p class="notifications-help"><?php _e('You can use this tags inside email content', 'easy-appointments');?> : <strong>#id#, #date#, #start#, #end#, #status#, #created#, #price#, #ip#, #link_confirm#, #link_cancel#, #service_name#, #service_duration#, #service_price#, #worker_name#, #worker_email#, #worker_phone#, #location_name#, #location_address#, #location_location#, <span id="custom-tags"></span></strong></p>
		<table class='notifications form-table'>
			<tbody>
				<tr>
					<td colspan="2">
						<p>
							<a class="mail-tab selected" data-textarea="#mail-pending"><?php _e('Pending', 'easy-appointments');?></a>
							<a class="mail-tab" data-textarea="#mail-reservation"><?php _e('Reservation', 'easy-appointments');?></a>
							<a class="mail-tab" data-textarea="#mail-canceled"><?php _e('Canceled', 'easy-appointments');?></a>
							<a class="mail-tab" data-textarea="#mail-confirmed"><?php _e('Confirmed', 'easy-appointments');?></a>
						</p>
		...
		<table class="form-table form-table-translation">
			<tbody>
				...
				<tr>
					...
					<td>
						<input style="width: 300px" class="field" data-key="pending.subject.visitor.email" name="pending.subject.visitor.email" type="text" value="<%= _.findWhere(settings, {ea_key:'pending.subject.visitor.email'}).ea_value %>"><br>
					</td>
					<td>
						<span class="description"> <?php _e('You can use any tag that is available as in custom email notifications.', 'easy-appointments');?></span>
					</td>
				...
				<tr>
					...
					<td>
						<input class="field" data-key="send.worker.email" name="send.worker.email" type="checkbox" <% if (_.findWhere(settings, {ea_key:'send.worker.email'}).ea_value == "1") { %>checked<% } %>><br>
					</td>
					<td>
		...
		<table class="form-table form-table-translation">
			<tbody>
				...
				<tr>
					...
					<td>
						<input class="field" data-key="block.time" name="block.time" type="text" value="<%= _.findWhere(settings, {ea_key:'block.time'}).ea_value %>"><br>
					</td>
					<td>
						<span class="description"> <?php _e('(in minutes). Prevent visitor to make appointment if there are less minutes that this.', 'easy-appointments');?></span>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					<td colspan="3">
						<span class="description"> <?php _e('For using the email notification for user there must be field named "email" or "e-mail"', 'easy-appointments');?></span>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					...
					<td>
						<input class="field" data-key="css.off" name="css.off" type="checkbox" <% if (_.findWhere(settings, {ea_key:'css.off'}).ea_value == "1") { %>checked<% } %>><br>
					</td>
				</tr>
			</tbody>
		</table>
		<table class="form-table">
			<tbody>
				...
				<tr>
					...
					<td>
						<input class="field" data-key="submit.redirect" name="submit.redirect" type="text" value="<%= _.findWhere(settings, {ea_key:'submit.redirect'}).ea_value %>"><br>
					</td>
					<td>
						<span class="description"> <?php _e('after visitor create appointment on front-end form. Leave blank to turn off redirect.', 'easy-appointments');?></span>
					</td>
				</tr>
			</tbody>
		</table>
